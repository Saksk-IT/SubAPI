    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          redeemCodeRebate: '兑换码参与返利',
          redeemCodeRebateHint: '开启后，被邀请用户使用正额余额兑换码充值时会产生邀请返利；并发和订阅兑换码不参与返利。',
          rebateRate: '首次付费返利比例',
          rebateRateHint: '被邀请用户首次符合返利条件的付费或充值所使用的比例（0-100%）。',
          repeatRebateRate: '持续复购返利比例',
          repeatRebateRateHint: '被邀请用户首次返利后的每次复购或充值所使用的比例（0-100%）。',
          ...
          customUsers: {
            title: '专属用户配置',
            description: '为指定用户设置专属邀请码或专属统一返利比例；专属比例同时覆盖首次与复购比例。仅展示已设置过专属配置的用户。',
            addButton: '添加专属用户',
          ...
          modal: {
            ...
            rateLabel: '专属统一返利比例（可选）',
            ratePlaceholder: '例如 30',
            rateHint: '0-100%，同时用于首次付费和持续复购；留空（编辑模式下）表示清除并沿用两档全局比例。',
