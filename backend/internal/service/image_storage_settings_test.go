...
func TestImageStorageSettingsDatabaseFailureDoesNotEnableFallback(t *testing.T) {
	svc, repo, built := newImageStorageFixture(t, config.ImageStorageConfig{
		Enabled: true, Endpoint: "https://acct.r2.cloudflarestorage.com", Region: "auto",
		Bucket: "yaml-bucket", AccessKeyID: "yaml-ak", SecretAccessKey: "yaml-sk",
	})
	...
	uploader, enabled := svc.resolve()
	require.False(t, enabled, "a settings read failure must fail closed instead of reviving the fallback")
	require.Nil(t, uploader)
	require.Empty(t, *built, "the storage factory must not receive fallback credentials after a database error")
}
