...
export const MAX_IMAGE_INPUT_FILE_BYTES = 20 * 1024 * 1024
export const MAX_MASK_EDIT_FILE_BYTES = MAX_IMAGE_INPUT_FILE_BYTES
export const MAX_IMAGE_INPUT_PAYLOAD_BYTES = 240 * 1024 * 1024
...
export function assertReferenceImageCount(count: number) {
  if (!Number.isSafeInteger(count) || count < 0 || count > MAX_REFERENCE_IMAGE_COUNT) {
    throw new Error(`参考图数量不能超过 ${MAX_REFERENCE_IMAGE_COUNT} 张`)
  }
}

export function assertImageInputFileSize(label: string, bytes: number) {
  assertMaxBytes(label, bytes, MAX_IMAGE_INPUT_FILE_BYTES)
}

export function assertImageInputFileSizes(inputImageDataUrls: string[]) {
  assertReferenceImageCount(inputImageDataUrls.length)
  inputImageDataUrls.forEach((dataUrl, index) => {
    assertImageInputFileSize(`参考图 ${index + 1}`, getDataUrlDecodedByteSize(dataUrl))
  })
}
