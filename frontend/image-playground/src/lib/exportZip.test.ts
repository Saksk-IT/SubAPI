...
describe('exportZip', () => {
  it('never writes legacy or profile API keys into exported ZIP settings', () => {
    const secretSettings = {
      apiKey: 'sk-legacy-secret',
      profiles: [
        { id: 'images', apiKey: 'sk-profile-secret' },
        { id: 'responses', apiKey: 'sk-response-secret' },
      ],
    } as AppSettings
    ...
    expect(JSON.stringify(readExportZip(bytes).manifest)).not.toContain('secret')
  })
