...
const nativeDialogMocks = vi.hoisted(() => ({ confirm: vi.fn() }))

vi.mock('@/services/nativeDialog', () => ({ nativeConfirm: nativeDialogMocks.confirm }))
...
describe('Prompt Audit components', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    nativeDialogMocks.confirm.mockReset().mockResolvedValue(true)
  })
  ...
  it('uses the shared danger confirmation before removing an endpoint', async () => {
    const wrapper = mount(EndpointPool, {
      props: { endpoints: [endpoint()], probeResults: {}, probingIds: [] },
      global: { stubs: { BaseDialog: DialogStub } },
    })
    ...
    nativeDialogMocks.confirm.mockResolvedValueOnce(false)
    await remove!.trigger('click')
    await flushPromises()
    expect(nativeDialogMocks.confirm).toHaveBeenCalledWith('admin.promptAudit.pool.deleteConfirm', { variant: 'danger' })
    expect(wrapper.emitted('update:endpoints')).toBeUndefined()

    nativeDialogMocks.confirm.mockResolvedValueOnce(true)
    await remove!.trigger('click')
    await flushPromises()
