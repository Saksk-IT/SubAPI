...
func TestAPIKeyAuthGrokVideoTaskReadsSkipBillingEnforcement(t *testing.T) {
	gin.SetMode(gin.TestMode)

	group := &service.Group{ID: 42, Status: service.StatusActive, Platform: service.PlatformGrok}
	user := &service.User{ID: 7, Role: service.RoleUser, Status: service.StatusActive, Balance: 0, Concurrency: 3}
	expiredAt := time.Now().Add(-time.Hour)
	apiKey := &service.APIKey{
		ID:        100,
		UserID:    user.ID,
		Key:       "grok-video-read-auth-only",
		Status:    service.StatusAPIKeyQuotaExhausted,
		User:      user,
		GroupID:   &group.ID,
		Group:     group,
	...
	for _, path := range []string{
		"/v1/videos/request-1",
		"/v1/videos/request-1/content",
		"/videos/request-1",
		"/videos/request-1/content",
	} {
...
func newAuthTestRouter(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) *gin.Engine {
	...
	router.POST("/v1/videos/generations", ok)
	router.GET("/v1/videos/:request_id", ok)
	router.GET("/v1/videos/:request_id/content", ok)
	router.GET("/videos/:request_id", ok)
	router.GET("/videos/:request_id/content", ok)
	return router
}
