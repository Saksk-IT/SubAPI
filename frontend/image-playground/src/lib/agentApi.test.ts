...
describe('callAgentResponsesApi', () => {
  ...
  it('rejects more than sixteen reference images across Agent request paths before fetch', async () => {
    ...
    const references = Array.from({ length: 17 }, (_, index) => `data:image/png;base64,${btoa(String(index))}`)
    ...
    await expect(callAgentResponsesApi({
      ...
      input: [{
        role: 'user',
        content: references.map((imageUrl) => ({ type: 'input_image', image_url: imageUrl })),
      }],
    })).rejects.toThrow('16')

    await expect(callAgentConversationTitleApi({
      settings: DEFAULT_SETTINGS,
      profile,
      prompt: 'title',
    ...
    await expect(callBatchImageSingle({
      profile,
      params: DEFAULT_PARAMS,
      batchItemId: 'batch-1',
      prompt: 'draw',
