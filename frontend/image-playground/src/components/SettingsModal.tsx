...
import { getManagedSnapshot, isManagedMode } from '../lib/managedMode'
...
export default function SettingsModal() {
  ...
  const [exportConfig, setExportConfig] = useState(!managedMode)
  const [exportTasks, setExportTasks] = useState(true)
  const [importConfig, setImportConfig] = useState(!managedMode)
  const [importTasks, setImportTasks] = useState(true)
  const [clearConfig, setClearConfig] = useState(!managedMode)
  ...
  const managedSnapshot = getManagedSnapshot()
  ...
  const updateManagedProfileModel = (profileId: string, model: string, commit = false) => {
    const nextDraft = {
      ...draft,
      profiles: draft.profiles.map((profile) => profile.id === profileId ? { ...profile, model } : profile),
    }
    setDraft(nextDraft)
  ...
  const handleClearAllData = async () => {
    const cleared = await clearData({ clearConfig, clearTasks })
    if (!cleared) return
    const nextDraft = normalizeSettings(useStore.getState().settings)
    setDraft(nextDraft)
    setTimeoutInput(String(getActiveApiProfile(nextDraft).timeout))
      ...
      >
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'agent' && (
              managedMode ? (
                <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4 text-sm text-blue-800 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
                  <h4 className="font-semibold">Sub2API 托管 Agent 配置</h4>
                  <p className="mt-2 leading-relaxed">托管模式下 Agent 固定使用 Responses + Images 双配置，凭证、接口地址和配置关系由主站统一管理。</p>
                </div>
              ) : (
                <AgentSettingsTab
                  draft={draft}
              ...
              managedMode ? (
                <div className="space-y-4">
                  <div className="rounded-2xl border border-blue-100 bg-blue-50/70 p-4 dark:border-blue-500/20 dark:bg-blue-500/10">
                    <h4 className="text-sm font-semibold text-blue-800 dark:text-blue-200">Sub2API 托管配置</h4>
                    <p className="mt-2 text-xs leading-relaxed text-blue-700 dark:text-blue-300">密钥由主站安全托管，不在此页面显示、编辑或复制。接口固定为同源 <code className="rounded bg-blue-100 px-1 py-0.5 dark:bg-blue-500/20">/v1</code>。</p>
                    <dl className="mt-3 grid gap-2 text-xs text-blue-700 dark:text-blue-300 sm:grid-cols-2">
                      <div>
                        <dt className="opacity-70">当前密钥</dt>
                        <dd className="mt-0.5 font-medium">{managedSnapshot?.config.apiKeyName ?? '主站密钥'}（ID {managedSnapshot?.config.apiKeyId ?? '-' }）</dd>
                      </div>
                      <div>
                        <dt className="opacity-70">API 地址</dt>
                        <dd className="mt-0.5 font-medium">/v1</dd>
                      </div>
                    </dl>
                  </div>

                  {draft.profiles.map((profile) => (
                    <label key={profile.id} className="block rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-white/[0.06] dark:bg-white/[0.02]">
                      <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">{profile.name}</span>
                      <span className="mt-1 block text-xs text-gray-500 dark:text-gray-400">{profile.apiMode === 'responses' ? 'Responses API' : 'Images API'} · 仅模型 ID 可编辑</span>
                      <input
                        value={profile.model}
                        onChange={(event) => updateManagedProfileModel(profile.id, event.target.value)}
                        onBlur={(event) => updateManagedProfileModel(profile.id, event.target.value, true)}
                        type="text"
                        maxLength={128}
                        aria-label={`${profile.name}模型 ID`}
                        className="mt-3 w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                      />
                    </label>
                  ))}
                </div>
