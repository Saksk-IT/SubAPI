...
func setupAdminRouter() (*gin.Engine, *stubAdminService) {
	...
	router.POST("/api/v1/admin/redeem-codes", redeemHandler.Create)
	router.PUT("/api/v1/admin/redeem-codes/:id", redeemHandler.Update)
	router.POST("/api/v1/admin/redeem-codes/generate", redeemHandler.Generate)
...
func TestRedeemHandlerEndpoints(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]any{"code": "R-CREATE", "type": "balance", "value": 10})
	rec = httptest.NewRecorder()
	...
	body, _ = json.Marshal(map[string]any{"code": "R-UPDATE", "status": "expired", "value": 10})
	rec = httptest.NewRecorder()
