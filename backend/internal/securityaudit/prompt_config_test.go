...
func TestConfigManagerSaveClearsUntrustedStateAfterInstallingSnapshot(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })

	currentJSON, err := json.Marshal(DefaultStorageConfig())
	require.NoError(t, err)
	mock.ExpectBegin()
	...
	mock.ExpectQuery(`SELECT value FROM settings WHERE key=\$1 FOR UPDATE`).
		WithArgs(SettingKeyPromptAuditConfig).
		WillReturnRows(sqlmock.NewRows([]string{"value"}).AddRow(string(currentJSON)))
	mock.ExpectExec(`INSERT INTO settings \(key,value,updated_at\) VALUES`).
		WithArgs(SettingKeyPromptAuditConfig, sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	manager := NewConfigManager(
		db,
		staticSettingRepository{values: map[string]string{SettingKeyRiskControl: "true"}},
		nil,
		prefixEncryptor{},
	)
	manager.configUntrusted.Store(true)
	manager.recordLoadError(errors.New("startup config unavailable"))

	request := promptAuditUpdateRequest(1, 1, "replacement-token")
	saved, err := manager.Save(context.Background(), request, 7)
	require.NoError(t, err)
	require.Equal(t, int64(2), saved.ConfigVersion)
