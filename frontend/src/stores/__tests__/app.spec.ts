...
import { APP_CONFIG_META_NAME } from '@/utils/injectedAppConfig'
...
function encodeMetaConfig(value: PublicSettings): string {
...
describe('useAppStore', () => {
  beforeEach(() => {
    ...
    document.querySelector(`meta[name="${APP_CONFIG_META_NAME}"]`)?.remove()
  ...
  describe('公开设置加载', () => {
    it('fetchPublicSettings 从 meta 注入配置返回 Promise 并跳过 API', async () => {
      ...
      meta.name = APP_CONFIG_META_NAME
      meta.content = encodeMetaConfig(settings)
      document.head.appendChild(meta)
      const store = useAppStore()

      const result = store.fetchPublicSettings()

      expect(result).toBeInstanceOf(Promise)
      await expect(result).resolves.toMatchObject({ site_name: 'Meta Site' })
      expect(getPublicSettings).not.toHaveBeenCalled()
      expect(store.siteName).toBe('Meta Site')
    })
