...
采用“公开功能开关 + 全局 Vue Launcher + 同源新标签页 + 一次性 MessageChannel”的结构：
...
- React 子应用通过 `window.opener` 完成一次性发现和端口转移，随后只使用独立 `MessagePort`。
...
- 父页严格校验 `event.origin`、`event.source === popup WindowProxy`、协议版本、nonce、精确字段集合和消息顺序。
- 子页严格校验 `event.origin`、`event.source === window.opener`、协议版本、nonce、精确字段集合和唯一端口。
- 子页收到 connect 后先接管 `MessagePort`，清空 `window.name`，设置 `window.opener = null`，再发送 connected。
- 父页只在 connected 后通过端口发送 configure；API 密钥只存在于父组件局部内存、MessagePort 队列和子应用受管运行时内存。
- 子页成功或失败时回传相同正整数 requestId；双方随后移除 window listener 并关闭不再需要的端口。
- 重复、越序、来源错误、nonce 错误、额外字段、无端口或多端口消息全部忽略并最终 fail-closed。
- 8 秒未完成、popup 被关闭或 opener 导航时清理会话；错误信息不得包含 API 密钥或内部异常详情。

`noopener`/`noreferrer` 不能用于握手创建阶段，因为它们会移除 opener；安全性通过固定同源 URL、短生命周期 opener、严格 source/origin 校验以及端口转移后立即断开 opener 获得。
...
- Launcher 关闭或失败时移除消息监听、清理定时器、关闭端口并丢弃局部密钥引用。
- 登录用户变化、登出或功能开关变为关闭时，立即关闭尚未完成的 Launcher 会话。
- 成功配置的新标签页不依赖主页面继续存在；主页面关闭后仍可使用，刷新子页则因凭据只在内存中而回到直达拒绝状态。
