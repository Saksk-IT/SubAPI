...
backend/migrations/182_prompt_audit.sql
backend/migrations/183_prompt_audit_full_prompt.sql
...
`181` 是提案编写时最大迁移号后的建议值。本 Fork 已使用 181，因此实际采用 `182_prompt_audit.sql` 和 `183_prompt_audit_full_prompt.sql`；不得改写已经应用的 migration。
...
1. 新增 `182_prompt_audit.sql`、`183_prompt_audit_full_prompt.sql` 以及 migration schema 测试。
