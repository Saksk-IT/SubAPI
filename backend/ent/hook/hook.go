...
// The BalanceProductFunc type is an adapter to allow the use of ordinary
// function as BalanceProduct mutator.
type BalanceProductFunc func(context.Context, *ent.BalanceProductMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BalanceProductFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.BalanceProductMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BalanceProductMutation", m)
}
