...
import { MAX_IMAGE_INPUT_FILE_BYTES } from './lib/imageApiShared'
vi.mock('./lib/db', () => {
...
describe('input image upload limits', () => {
  it('rejects an oversized file before reading or storing it', async () => {
    const oversizedFile = {
      type: 'image/png',
      size: MAX_IMAGE_INPUT_FILE_BYTES + 1,
    } as File

    await expect(createInputImageFromFile(oversizedFile)).rejects.toThrow('20.0 MiB')
  })

  it('rejects an oversized URL response from its content length before conversion', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('x', {
      status: 200,
      headers: {
        'Content-Type': 'image/png',
        'Content-Length': String(MAX_IMAGE_INPUT_FILE_BYTES + 1),
      },
    }))

    await expect(addImageFromUrl('https://images.example/oversized.png')).rejects.toThrow('20.0 MiB')
...
describe('input persistence setting', () => {
  ...
  it('never persists legacy or profile API keys to localStorage state', () => {
    const settings = normalizeSettings({
      ...DEFAULT_SETTINGS,
      apiKey: 'sk-legacy-secret',
      profiles: [
        createDefaultOpenAIProfile({ id: 'images', apiKey: 'sk-images-secret' }),
        createDefaultOpenAIProfile({ id: 'responses', apiKey: 'sk-responses-secret', apiMode: 'responses' }),
      ],
      activeProfileId: 'images',
    })
    useStore.setState({
      settings,
      dismissedCodexCliPrompts: ['/v1\nsk-images-secret'],
    })
    ...
    expect(persisted.dismissedCodexCliPrompts).toEqual([])
    expect(JSON.stringify(persisted)).not.toContain('secret')
  })

  it('does not derive a persisted prompt preference key from the API secret', () => {
    const settings = normalizeSettings({
      ...DEFAULT_SETTINGS,
      profiles: [createDefaultOpenAIProfile({ id: 'images', apiKey: 'sk-prompt-secret' })],
      activeProfileId: 'images',
    })

    expect(getCodexCliPromptKey(settings)).not.toContain('sk-prompt-secret')
  })

  it('removes API keys from legacy persisted settings during migration', () => {
    const migrated = migratePersistedState({
      settings: {
        ...DEFAULT_SETTINGS,
        apiKey: 'sk-legacy-secret',
        profiles: [createDefaultOpenAIProfile({ apiKey: 'sk-profile-secret' })],
      },
    })
  ...
  it('prevents store updates from replacing managed credentials or providers', () => {
    ...
    useStore.getState().setSettings({
      ...
      profiles: [createDefaultFalProfile({ apiKey: 'fal-secret' })],
    })
