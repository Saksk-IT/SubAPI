...
type firstRechargeMarkCompletedFailingRepo struct {
	*firstRechargeMemoryRepo
	err error
}

func (r *firstRechargeMarkCompletedFailingRepo) MarkCompleted(context.Context, int64, int64, time.Time) error {
	return r.err
}
...
func TestMarkCompletedPreservesFirstRechargeStateWithFulfillmentLease(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	ensurePaymentAuditOrderActionUniqueIndex(t, ctx, client)
	...
	order, err := client.PaymentOrder.UpdateOneID(order.ID).
		SetActivityType(FirstRechargeActivityType).
	...
	state, err := firstRechargeRepo.GetUserState(ctx, order.UserID)
	require.NoError(t, err)
	require.NotNil(t, state)
	require.NotNil(t, state.CompletedAt)
	require.NotNil(t, state.CompletedOrderID)
	require.Equal(t, order.ID, *state.CompletedOrderID)
}

func TestMarkCompletedRollsBackFirstRechargeOrderAndAllowsRetry(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	ensurePaymentAuditOrderActionUniqueIndex(t, ctx, client)
	staleAt := time.Now().Add(-paymentFulfillmentLeaseDuration - time.Minute)
	...
	require.ErrorContains(t, err, "first recharge state unavailable")
	reloaded, getErr := client.PaymentOrder.Get(ctx, order.ID)
	require.NoError(t, getErr)
	require.Equal(t, OrderStatusRecharging, reloaded.Status)
	require.Nil(t, reloaded.CompletedAt)

	svc.markFailed(ctx, order.ID, lease, err)
	reloaded, getErr = client.PaymentOrder.Get(ctx, order.ID)
	require.NoError(t, getErr)
	...
	require.NoError(t, svc.markCompleted(ctx, reloaded, retryLease, "SUBSCRIPTION_SUCCESS"))

	reloaded, getErr = client.PaymentOrder.Get(ctx, order.ID)
	require.NoError(t, getErr)
	require.Equal(t, OrderStatusCompleted, reloaded.Status)
	state, stateErr := firstRechargeRepo.GetUserState(ctx, order.UserID)
