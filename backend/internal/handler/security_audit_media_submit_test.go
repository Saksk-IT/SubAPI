...
func TestOpenAIImageJobPromptGuardRunsBeforePersistence(t *testing.T) {
	gin.SetMode(gin.TestMode)
	created := false
	repository := &openAIImageJobHandlerRepository{
	...
	engine := blockingHandlerPromptEngine()
	openAI := &OpenAIGatewayHandler{securityAuditCoordinator: securityaudit.NewCoordinator(nil, engine)}
	h := NewOpenAIImageJobHandler(repository, &service.OpenAIGatewayService{}, openAIImageJobHandlerTestConfig(), nil)
	h.openAI = openAI

	router := gin.New()
	router.Use(securityAuditMediaTestMiddleware)
	router.POST("/v1/images/generations/jobs", h.Submit)
	request := httptest.NewRequest(http.MethodPost, "/v1/images/generations/jobs", strings.NewReader(`{"model":"gpt-image-2","prompt":"blocked durable prompt"}`))
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("Idempotency-Key", "blocked-durable-job")
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)

	require.Equal(t, http.StatusForbidden, recorder.Code)
	require.Contains(t, recorder.Body.String(), securityaudit.ErrorCodeBlocked)
	require.False(t, created, "no durable image job may exist after a blocking decision")
	evaluated, _, requests := engine.snapshot()
	require.Equal(t, 1, evaluated)
	require.Len(t, requests, 1)
	require.Contains(t, string(requests[0].Body), "blocked durable prompt")
}
