<template>
  <AppLayout>
    ...
    <div
      ...
      data-test="save-bar"
      class="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/95 px-4 py-3 shadow-[0_-12px_35px_rgba(15,23,42,0.08)] backdrop-blur dark:border-dark-700/80 dark:bg-dark-900/95 dark:shadow-[0_-12px_35px_rgba(0,0,0,0.35)]"
      :class="appStore.sidebarCollapsed ? 'lg:left-[72px]' : 'lg:left-64'"
    >
...
async function loadEvents() {
  ...
  loadErrors.events = ''
  try {
    const result = await promptAuditAPI.listEvents(requestFilters, page, pageSize)
    ...
    if (generation === eventsGeneration) loading.events = false
  }
}
...
async function openEvent(id: number) {
  const generation = ++eventDetailGeneration
  showEventDetail.value = true
  loading.detail = true
  activeEvent.value = null
  try {
    const event = await promptAuditAPI.getEvent(id)
    if (generation === eventDetailGeneration) activeEvent.value = event
  } catch (error) {
    if (generation !== eventDetailGeneration) return
    appStore.showError(errorMessage(error, 'admin.promptAudit.errors.loadDetail'))
    showEventDetail.value = false
  } finally {
    if (generation === eventDetailGeneration) loading.detail = false
  }
}
function closeEventDetail() {
  eventDetailGeneration += 1
  showEventDetail.value = false
...
async function runFilterDeletePreview(value: PromptEventFilters) {
  ...
  try {
    const preview = await promptAuditAPI.previewDelete(value)
...
async function confirmFilterDelete() {
  ...
  if (!preview || !previewFilters || preview.matched_count <= 0) return
  loading.deleting = true
  try {
