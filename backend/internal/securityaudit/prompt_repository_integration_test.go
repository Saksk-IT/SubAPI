...
func TestPromptAuditRepositoryAdmissionClaimFencingAndEventTransaction(t *testing.T) {
	...
	_, err = repo.Complete(ctx, firstClaim, firstClaim.ConfigVersion, integrationResult(EventCritical), true)
	require.ErrorIs(t, err, ErrLeaseLost)

	event, err := repo.Complete(ctx, secondClaim, secondClaim.ConfigVersion, integrationResult(EventCritical), true)
	require.NoError(t, err)
...
func TestPromptAuditRepositoryFilterDeleteWaitsForExternallyLockedMatch(t *testing.T) {
	db := openPromptAuditIntegrationDB(t)
	repo := NewPostgreSQLRepository(db)
	ctx := context.Background()
	now := time.Now().UTC()
	...
	lockedEvent, err := repo.RecordBlocking(ctx, integrationSnapshot("locked-first"), 1, integrationResult(EventCritical), true)
	require.NoError(t, err)
	for _, seed := range []string{"locked-second", "locked-third"} {
		_, err := repo.RecordBlocking(ctx, integrationSnapshot(seed), 1, integrationResult(EventCritical), true)
		require.NoError(t, err)
	}
	preview, err := repo.PreviewDelete(ctx, filter)
	require.NoError(t, err)
	...
	require.NoError(t, lockTx.QueryRowContext(ctx,
		`SELECT id FROM prompt_audit_events WHERE id=$1 FOR UPDATE`, lockedEvent.ID,
	).Scan(&selectedID))
	require.Equal(t, lockedEvent.ID, selectedID)
	...
	defer cancel()
	outcomes := make(chan deleteOutcome, 1)
	go func() {
		result, deleteErr := repo.DeleteEventsByFilter(deleteCtx, filter, preview.SnapshotMaxID, 1)
		outcomes <- deleteOutcome{result: result, err: deleteErr}
	}()

	select {
	case outcome := <-outcomes:
		_ = lockTx.Rollback()
		require.FailNowf(t, "filter delete returned before the matching row lock was released", "result=%+v err=%v", outcome.result, outcome.err)
	...
	case outcome := <-outcomes:
		...
		require.Equal(t, int64(3), outcome.result.DeletedEvents)
	case <-deleteCtx.Done():
		require.FailNow(t, "filter delete did not resume after the external row lock was released")
	}

	remaining, err := repo.ListEvents(ctx, filter, 1, 100)
	require.NoError(t, err)
	require.Zero(t, remaining.Total, "all matching rows within the preview high-water mark must be deleted")
}
