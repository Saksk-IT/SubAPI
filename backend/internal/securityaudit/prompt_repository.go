...
type JobRepository interface {
	...
	Complete(ctx context.Context, job *Job, executedConfigVersion int64, result *NormalizedResult, storePassEvents bool) (*Event, error)
...
func (r *PostgreSQLRepository) Complete(ctx context.Context, job *Job, executedConfigVersion int64, result *NormalizedResult, storePassEvents bool) (*Event, error) {
	...
	if shouldStorePromptAuditEvent(result.Decision, storePassEvents) {
		event, err = insertEvent(ctx, tx, job.ID, job.Snapshot.Redacted(), executedConfigVersion, result)
		if err != nil {
			return nil, err
		}
	}
