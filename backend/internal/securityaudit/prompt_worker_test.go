...
func (r *fakeJobRepository) Complete(_ context.Context, _ *Job, executedConfigVersion int64, result *NormalizedResult, storePass bool) (*Event, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.completeCount++
	r.completedResult, r.completedConfig, r.completedStore = result, executedConfigVersion, storePass
...
func TestWorkerCompletesPassWithoutEventRefreshesEveryChunkAndDeletesPayload(t *testing.T) {
	...
	require.Equal(t, int64(11), repo.completedConfig, "event must record the config snapshot used for scanning")
