export default {
  ...
  payment: {
    ...
    purchaseGuide: {
      eyebrow: 'API 充值',
      title: '选择适合你的 API 使用方式',
      subtitle: '这里面向 API 调用客户展示可购买的充值商品与订阅套餐，购买成功后权益会自动发放到账户。',
      apiRechargeTitle: '面向客户的充值商品',
      apiRechargeDescription: '充值商品适合按需调用 API。购买后会增加账户余额或额度，可用于后续按实际请求消耗扣费。',
      rechargeTitle: '充值模式',
      rechargeDescription: '先充值到账户余额，再按模型、倍率或请求用量扣除。适合用量波动大、希望灵活控制成本的场景。',
      subscriptionTitle: '订阅模式',
      subscriptionDescription: '按套餐周期获得指定分组、额度或有效期权益。适合固定周期使用、需要稳定配额的客户。',
      billingTitle: '计费模式区别',
      billingRecharge: '充值：按实际 API 消耗从余额中扣费，余额未用完会继续保留。',
      billingSubscription: '订阅：按套餐有效期计费，周期内享受套餐内配额、倍率或访问权限。',
      contactEyebrow: '客服支持',
      contactTitle: '购买前后都可以联系客服',
      contactSubtitle: '二维码、文字说明和联系方式由管理员在后台自定义，适合放置微信、QQ、邮箱或工单入口。',
      contactQr: '扫码联系客服',
      contactInfo: '联系方式',
      helpTitle: '补充说明',
