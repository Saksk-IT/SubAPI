...
// 回归：本地多图拆分路径也必须用排除心跳字节后的响应大小做错误判断。
// 否则并发批次返回可重试错误时，心跳会被误判为真实响应并阻止账号切换。
func TestOpenAIImagesJSONKeepalive_HeartbeatBeforeOAuthBatchStillFailsOver(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"draw two cats","response_format":"b64_json","n":2}`)
	...
	newFailureResponse := func(requestID string) *http.Response {
	...
	svc := &OpenAIGatewayService{
		httpUpstream: &httpUpstreamRecorder{responses: []*http.Response{
			newFailureResponse("req_img_batch_failover_1"),
			newFailureResponse("req_img_batch_failover_2"),
		}},
	}
	parsed, err := svc.ParseOpenAIImagesRequest(c, body)
	require.NoError(t, err)
	...
	account := &Account{
		ID:       23,
		Name:     "openai-oauth-batch-heartbeat-failover",
		Platform: PlatformOpenAI,
		Type:     AccountTypeOAuth,
		Credentials: map[string]any{
			"access_token": "token-123",
