...
func (s *PaymentService) markCompleted(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease, auditAction string) error {
	...
	if o.ActivityType == FirstRechargeActivityType && s.firstRechargeService != nil {
		return s.markFirstRechargeCompleted(ctx, o, lease, auditAction)
	}
	completed, err := updatePaymentOrderCompleted(ctx, s.entClient, o, lease)
	if err != nil {
		return err
	}
	if completed {
		s.recordPaymentFulfillmentCompleted(ctx, o, auditAction)
	}
	return nil
}

func updatePaymentOrderCompleted(ctx context.Context, client *dbent.Client, o *dbent.PaymentOrder, lease *paymentFulfillmentLease) (bool, error) {
	now := time.Now()
...
func (s *PaymentService) markFirstRechargeCompleted(ctx context.Context, o *dbent.PaymentOrder, lease *paymentFulfillmentLease, auditAction string) error {
	tx, err := s.entClient.Tx(ctx)
	if err != nil {
		return fmt.Errorf("begin first recharge completion tx: %w", err)
	}
	defer func() { _ = tx.Rollback() }()

	txCtx := dbent.NewTxContext(ctx, tx)
	completed, err := updatePaymentOrderCompleted(txCtx, tx.Client(), o, lease)
	if err != nil || !completed {
		return err
	}
	if err := s.firstRechargeService.MarkCompleted(txCtx, o.UserID, o.ID); err != nil {
		_ = tx.Rollback()
		s.writeAuditLog(ctx, o.ID, "FIRST_RECHARGE_STATE_FAILED", "system", map[string]any{"error": err.Error()})
		return fmt.Errorf("mark first recharge completed: %w", err)
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit first recharge completion: %w", err)
	}
	s.recordPaymentFulfillmentCompleted(ctx, o, auditAction)
	return nil
}

func (s *PaymentService) recordPaymentFulfillmentCompleted(ctx context.Context, o *dbent.PaymentOrder, auditAction string) {
