...
function collectAgentInputImageDataUrls(input: unknown): string[] {
  const images: string[] = []
  ...
  const visit = (value: unknown): void => {
    ...
    if (typeof record.image_url === 'string' && record.image_url.startsWith('data:image/')) {
      images.push(record.image_url)
    }
    for (const [key, child] of Object.entries(record)) {
      if (key !== 'image_url') visit(child)
    }
  }
...
function validateAgentImagePayload(imageDataUrls: string[], maskDataUrl?: string): void {
  assertImageInputFileSizes(imageDataUrls)
  if (maskDataUrl) {
    assertMaskEditFileSize('遮罩文件', getDataUrlDecodedByteSize(maskDataUrl))
  }
  assertImageInputPayloadSize(
    imageDataUrls.reduce((sum, dataUrl) => sum + getDataUrlEncodedByteSize(dataUrl), 0) +
...
export async function callAgentResponsesApi(opts: {
  ...
  validateAgentImagePayload(collectAgentInputImageDataUrls(input), maskDataUrl)
  ...
  try {
    ...
    const response = await managedFetch(buildApiUrl(profile.baseUrl, 'responses', proxyConfig, useApiProxy), {
      method: 'POST',
...
export async function callAgentConversationTitleApi(opts: {
  ...
  validateAgentImagePayload(imageDataUrls ?? [])
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  const controller = new AbortController()
  const timeoutId = setTimeout(() => controller.abort(), profile.timeout * 1000)
...
export async function callBatchImageSingle(opts: {
  ...
  validateAgentImagePayload(referenceImageDataUrls)
  const mime = MIME_MAP[params.output_format] || 'image/png'
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  const controller = new AbortController()
