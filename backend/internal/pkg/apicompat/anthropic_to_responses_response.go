...
func anthToResHandleContentBlockStart(evt *AnthropicStreamEvent, state *AnthropicEventToResponsesState) []ResponsesStreamEvent {
	...
	case "thinking":
		// A message item intentionally stays open across consecutive text
		// blocks. A different block type starts a new output item, so close the
		// message before replacing the current item state.
		events = append(events, closeCurrentResponsesItem(state)...)
