...
func TestPromptAdminEventDetailDisablesCachingForFullPrompt(t *testing.T) {
	service := &fakePromptAdminService{get: func(_ context.Context, id int64) (*Event, error) {
		require.Equal(t, int64(42), id)
		return &Event{ID: id, Snapshot: PromptSnapshot{FullPrompt: "sensitive full prompt"}}, nil
	}}

	response := promptAdminRequest(t, promptAdminRouter(service), http.MethodGet, "/admin/prompt-audit/events/42", nil)
	require.Equal(t, http.StatusOK, response.Code)
	require.Equal(t, "no-store", response.Header().Get("Cache-Control"))
	require.Equal(t, "no-cache", response.Header().Get("Pragma"))
	require.Contains(t, response.Body.String(), "sensitive full prompt")
}
