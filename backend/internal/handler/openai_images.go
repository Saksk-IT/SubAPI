...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		if !parsed.Stream && !jsonKeepaliveStarted {
			stopJSONKeepalive = service.StartOpenAIImagesJSONKeepalive(c, h.openAIImagesJSONKeepaliveInterval(c))
		...
		h.submitOpenAIImageUsageRecordTask(c.Request.Context(), func(ctx context.Context) error {
...
func (h *OpenAIGatewayHandler) openAIImagesJSONKeepaliveInterval(c *gin.Context) time.Duration {
	if c != nil && c.Request != nil {
		if _, ok := service.OpenAIImageJobExecutionObserverFromContext(c.Request.Context()); ok {
			return 0
		}
	}
