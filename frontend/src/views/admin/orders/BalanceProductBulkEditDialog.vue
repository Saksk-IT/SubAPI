<template>
  <BaseDialog :show="show" :title="t('payment.admin.bulkEditBalanceProductsTitle')" width="wide" @close="emit('close')">
    <form id="balance-product-bulk-edit-form" class="space-y-4" @submit.prevent="handleSubmit">
      <div class="rounded-lg bg-primary-50 px-3 py-2 text-sm font-medium text-primary-900 dark:bg-primary-900/20 dark:text-primary-100">
        {{ t('payment.admin.bulkEditBalanceProductsSelected', { count: productIds.length }) }}
      </div>
      ...
      <div class="space-y-3">
        <label class="flex items-start gap-3 rounded-lg border border-gray-200 p-3 dark:border-dark-600">
          ...
          <div class="min-w-0 flex-1">
            <span class="input-label mb-1 block">{{ t('payment.admin.productDescription') }}</span>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <button type="submit" form="balance-product-bulk-edit-form" :disabled="saving || productIds.length === 0" class="btn btn-primary">
...
async function handleSubmit() {
  ...
  if (Object.keys(fields).length === 0) {
    appStore.showError(t('payment.admin.bulkEditBalanceProductsNoFields'))
    return
  }
  saving.value = true
  try {
    const res = await adminPaymentAPI.bulkUpdateBalanceProducts({
      product_ids: props.productIds,
      fields,
    })
    appStore.showSuccess(t('payment.admin.bulkEditBalanceProductsSuccess', { count: res.data.updated }))
    emit('updated')
  } catch (err: unknown) {
