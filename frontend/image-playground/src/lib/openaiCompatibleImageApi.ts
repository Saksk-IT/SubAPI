...
export async function callOpenAICompatibleImageApi(opts: CallApiOptions, profile: ApiProfile, customProvider?: CustomProviderDefinition | null): Promise<CallApiResult> {
  assertImageInputFileSizes(opts.inputImageDataUrls)
  if (opts.maskDataUrl) {
...
async function callImagesApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    if (isEdit) {
      ...
      response = await managedFetch(buildApiUrl(profile.baseUrl, paths.editPath, proxyConfig, useApiProxy), {
        method: 'POST',
      ...
      response = await managedFetch(buildApiUrl(profile.baseUrl, paths.generationPath, proxyConfig, useApiProxy), {
        method: 'POST',
        headers: {
          ...requestHeaders,
...
async function pollCustomTaskResult(
  ...
  while (true) {
    ...
    try {
      const taskResponse = await managedFetch(buildApiUrl(profile.baseUrl, taskPath, proxyConfig, false), {
