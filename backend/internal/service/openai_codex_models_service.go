...
// may succeed without changing the request. Account-specific OAuth credential
// failures are retried so one stale account cannot block the whole group;
// request-wide configuration and upstream 4xx responses, except 429, are not.
func IsRetryableCodexModelsManifestError(err error) bool {
	...
	return IsCodexModelsManifestAccountError(err)
}

// IsCodexModelsManifestAccountError reports failures isolated to one selected
// account. Handlers may exclude that account without consuming the transient
// upstream switch budget, preserving access to later valid accounts in a
// mixed OAuth/API-key group.
func IsCodexModelsManifestAccountError(err error) bool {
