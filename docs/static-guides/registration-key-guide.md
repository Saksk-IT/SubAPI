...
本教程适用于小铺购买的额度或订阅兑换码、站内充值或订阅。

API 密钥分组选择与计费模式：

| 计费模式 | 密钥分组 | 额度规则 |
| --- | --- | --- |
| 订阅模式 | 选择相应订阅分组（订阅 xx 刀） | 每天有固定额度，通常在每天 0 点后刷新。 |
| 额度模式 | 选择相应消耗额度的分组（OpenAI-Plus、Pro、Claude 等） | 调用时按实际消耗从网站账户余额中扣费。 |

需要购买兑换码时，可使用小铺卡密自助购买地址：<https://catfk.com/shop/92O8CR0C>
...
3. 根据计费模式选择对应的订阅分组，或 `OpenAI-Plus`、`Pro`、`Claude` 等额度分组；具体名称以中转后台实际显示为准。
4. 保存后回到密钥列表。

![API 密钥页面暂无密钥并显示创建密钥按钮](../../frontend/public/img/codex-guide/image-18.png)

图 3：密钥列表为空时，点击页面中的“创建密钥”。

![创建密钥弹窗中选择 API 分组](../../frontend/public/img/codex-guide/image-15.png)

图 4：分组决定密钥使用的权益来源，选错会导致可用额度异常。

![密钥列表使用密钥入口](../../frontend/public/img/codex-guide/image-20.png)

图 5：创建后回到密钥列表，点击“使用密钥”。
