...
import { enforceManagedSettings, getManagedStorageName, redactSettingsSecrets } from './lib/managedMode'
...
export function migratePersistedState(persistedState: unknown): unknown {
  ...
  return {
    ...
    ...(persistedState.settings
      ? { settings: redactSettingsSecrets(normalizeSettings(persistedState.settings)) }
      : {}),
...
export function getPersistedState(state: AppState) {
  const runtimeSettings = normalizeSettings(state.settings)
  const settings = redactSettingsSecrets(runtimeSettings)
  const apiKeys = [runtimeSettings.apiKey, ...runtimeSettings.profiles.map((profile) => profile.apiKey)]
    .filter((apiKey) => apiKey.length > 0)
  const dismissedCodexCliPrompts = state.dismissedCodexCliPrompts
    .filter((promptKey) => apiKeys.every((apiKey) => !promptKey.includes(apiKey)))
  const galleryInputDraft = getPersistableGalleryInputDraft(state)
  return {
    settings,
    params: state.params,
    ...(runtimeSettings.persistInputOnRestart && (state.appMode === 'gallery' || galleryInputDraft)
      ? {
    ...
    galleryInputDraft: runtimeSettings.persistInputOnRestart && galleryInputDraft
  ...
  persist(
    (set, get) => ({
      ...
      setSettings: (s) => set((st) => {
        ...
        const settings = enforceManagedSettings(normalizeSettings(merged))
...
export async function createInputImageFromFile(file: File): Promise<InputImage | null> {
  ...
  assertImageInputFileSize('参考图文件', file.size)
...
export async function addImageFromUrl(src: string): Promise<void> {
  ...
  const blob = await readBoundedInputImageResponse(res)
...
async function readBoundedInputImageResponse(response: Response): Promise<Blob> {
  if (!response.ok) throw new Error(`图片读取失败：HTTP ${response.status}`)
  const contentType = response.headers.get('Content-Type')?.split(';', 1)[0]?.trim() ?? ''
  if (!contentType.startsWith('image/')) throw new Error('不是有效的图片')

  const rawContentLength = response.headers.get('Content-Length')
  if (rawContentLength) {
    const contentLength = Number(rawContentLength)
    if (Number.isFinite(contentLength) && contentLength >= 0) {
      assertImageInputFileSize('参考图文件', contentLength)
    }
  }

  if (!response.body) {
    const blob = await response.blob()
    assertImageInputFileSize('参考图文件', blob.size)
    return blob
  }
  ...
  try {
    while (true) {
      ...
      assertImageInputFileSize('参考图文件', totalBytes)
