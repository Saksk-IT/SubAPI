...
func TestCodexIdentityKeysPreferStrongIdentifiers(t *testing.T) {
	keys := buildCodexImportIdentityKeys("acct-1", "acct-user-1", "user-1", "org-1", "same@example.com", "token", "refresh")
	...
	hasAccountUser := false
	for _, key := range keys {
		if key == "account_user:acct-1:acct-user-1" {
			hasAccountUser = true
		}
		if strings.HasPrefix(key, "email:") || strings.HasPrefix(key, "email_org:") {
			t.Fatalf("strong identity should not include email fallback: %v", keys)
		}
	}
	if !hasAccountUser {
		t.Fatalf("strong identity should include account_user fallback: %v", keys)
	}

	keys = buildCodexImportIdentityKeys("", "", "", "org-1", "same@example.com", "token", "refresh")
	hasEmailOrg := false
	for _, key := range keys {
	...
	keys = buildCodexImportIdentityKeys("acct-1", "acct-user-1", "user-1", "org-1", "same@example.com", "token", "")
...
func TestNormalizeCodexImportEntriesDoNotDuplicateDifferentAccountUsersInSharedWorkspace(t *testing.T) {
	...
	markCodexIdentitySeen(seen, first.IdentityKeys, 1, first.UserID)
	if duplicateIndex, ok := firstSeenCodexIdentity(seen, second.IdentityKeys, second.UserID); ok {
