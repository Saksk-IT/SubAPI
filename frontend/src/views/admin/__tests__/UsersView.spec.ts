...
const DataTableStub = {
  ...
  template: `
    <div>
      ...
      <div data-test="table-selectable">{{ String(selectable) }}</div>
...
describe('admin UsersView', () => {
  ...
  it('keeps bulk-limit selection separate from batch-delete selection', async () => {
    const wrapper = mount(UsersView, {
      global: {
        stubs: {
          AppLayout: { template: '<div><slot /></div>' },
    ...
    await flushPromises()
    expect(wrapper.get('[data-test="table-selectable"]').text()).toBe('true')

    await wrapper.get('[data-test="select-42"]').trigger('click')
    expect(wrapper.find('[data-test="bulk-edit-limits"]').exists()).toBe(true)

    await wrapper.get('[data-test="batch-delete-mode"]').trigger('click')
    expect(wrapper.get('[data-test="table-selectable"]').text()).toBe('false')
    expect(wrapper.find('[data-test="bulk-edit-limits"]').exists()).toBe(false)
    expect(wrapper.get('[data-test="selected-keys"]').text()).toBe('42')
  })
})
