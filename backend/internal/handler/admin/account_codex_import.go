...
func normalizeCodexImportEntry(entry codexImportEntry) (*codexImportAccount, error) {
	...
	item.IdentityKeys = buildCodexImportIdentityKeys(item.AccountID, item.AccountUserID, item.UserID, item.Organization, item.Email, item.AccessToken, item.RefreshToken)
...
func buildCodexImportIdentityKeys(accountID, accountUserID, userID, organizationID, email, accessToken, refreshToken string) []string {
	accessToken = strings.TrimSpace(accessToken)
	refreshToken = strings.TrimSpace(refreshToken)
	if refreshToken == "" && accessToken != "" {
		return []string{"access:" + codexTokenFingerprint(accessToken)}
	}
	return buildCodexStoredIdentityKeys(accountID, accountUserID, userID, organizationID, email, accessToken, refreshToken)
}

// buildCodexStoredIdentityKeys 生成存量账号索引键，保留 user/account 维度，
// 让 accessToken-only 账号后续升级为完整 OAuth 时仍能命中并更新原账号。
func buildCodexStoredIdentityKeys(accountID, accountUserID, userID, organizationID, email, accessToken, refreshToken string) []string {
