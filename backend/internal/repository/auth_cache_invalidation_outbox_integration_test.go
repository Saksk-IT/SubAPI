...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	require.Equal(t, 1, count(), "key creation must invalidate negative caches on every instance")
	clear()
	t.Cleanup(clear)
	t.Cleanup(func() {
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "group capability changes must invalidate cached authorization snapshots")
	clear()
