...
func TestIsRetryableCodexModelsManifestCredentialError(t *testing.T) {
	for _, reason := range []string{
		"OPENAI_CODEX_MODELS_CREDENTIALS_FAILED",
		...
		t.Run(reason, func(t *testing.T) {
			err := infraerrors.New(http.StatusBadGateway, reason, "account-specific credential failure")
			if !IsRetryableCodexModelsManifestError(err) {
				t.Fatalf("expected %s to be retryable on another account", reason)
			}
		})
	}
}
