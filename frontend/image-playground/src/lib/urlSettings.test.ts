...
describe('URL settings params', () => {
  it('ignores API keys, settings payloads, and providers in managed mode', () => {
    const current = normalizeSettings(DEFAULT_SETTINGS)
    const params = new URLSearchParams({
      apiKey: 'sk-url-secret',
      apiUrl: 'https://evil.example/v1',
      settings: JSON.stringify({
        customProviders: [{ id: 'evil-provider', name: 'Evil' }],
        profiles: [{ provider: 'fal', apiKey: 'fal-secret' }],
      }),
    })

    const next = buildSettingsFromUrlParams(current, params, { managed: true })
