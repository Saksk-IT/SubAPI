...
//
// /v1/usage、/v1/sub2api/billing 端点、异步生图任务查询与 Grok 视频任务查询
// 只需鉴权，不需要计费执行。
// usage 允许过期/配额耗尽的 Key 查询自身用量，billing 用于读取当前 Key 的倍率配置，
// 异步生图与 Grok 视频查询允许已耗尽额度的 Key 拉取自身任务结果。
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		skipBilling := c.Request.URL.Path == "/v1/usage" || billingInfoRequest ||
			...
			isGrokVideoTaskRead(c.Request.Method, c.Request.URL.Path)
...
func isGrokVideoTaskRead(method, path string) bool {
	...
	requestID := strings.TrimSpace(segments[1])
	if requestID == "" {
		return false
	}

	return len(segments) == 2 || (len(segments) == 3 && segments[2] == "content")
}
