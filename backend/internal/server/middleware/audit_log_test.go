...
func TestAuditSensitiveReadsIncludesDataManagementS3Profiles(t *testing.T) {
	const route = "GET /api/v1/admin/data-management/s3/profiles"
	if got := auditSensitiveReads[route]; got != "admin.data_management.s3_profiles.read" {
		t.Fatalf("auditSensitiveReads[%q] = %q", route, got)
	}
	if _, exists := auditSensitiveReads["GET /api/v1/admin/data-management/s3/config"]; exists {
		t.Fatal("auditSensitiveReads must not contain the nonexistent data-management S3 config route")
	}
}
...
func TestPromptAuditAdminOperationsUseOmittedBodiesAndAllowlistedDetails(t *testing.T) {
	...
	router.GET("/api/v1/admin/prompt-audit/events/:id", func(c *gin.Context) {
		SetAuditExtra(c, map[string]any{"result": "success", "event_id": int64(42)})
		c.JSON(http.StatusOK, gin.H{"full_prompt": "audit-canary-prompt"})
	})
	...
	for _, request := range []*http.Request{
		...
		httptest.NewRequest(http.MethodGet, "/api/v1/admin/prompt-audit/events/42", nil),
	} {
	...
	require.Equal(t, "<credential-bearing body omitted>", config.RequestBody)
	...
	require.Equal(t, "<credential-bearing body omitted>", probe.RequestBody)
	...
	detail := byAction["admin.prompt_audit.event.read"]
	require.NotNil(t, detail)
	require.Equal(t, http.StatusOK, detail.StatusCode)
	require.Empty(t, detail.RequestBody)
	require.Equal(t, "success", detail.Extra["result"])
	require.EqualValues(t, 42, detail.Extra["event_id"])
}

func TestPromptAuditMutationAuditRoutesHaveStableActionsAndOmitBodies(t *testing.T) {
	require.Equal(
		t,
		"admin.prompt_audit.event.read",
		auditSensitiveReads["GET /api/v1/admin/prompt-audit/events/:id"],
	)
